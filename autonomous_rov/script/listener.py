...
class GenerateTraj:
    def __init__(self):
        ...
        self.set_point_pub = rospy.Publisher(
            "pid/depth/setpoint", Float64, queue_size=1)
        self.curr_depth = 0
    ...
    def generate_traj(self, end, start=0):
        robot_path = np.array([start, end])
        # print(robot_path)
        my_robot = stopp.Robot(
            n_joints=1, j_max=self.j_max, a_max=self.a_max, v_max=self.v_max)
        trajectory = my_robot.TimeParameterizePath(
            robot_path, interp_time_step=self.dt)
...
def subscriber():
    ...
    rospy.Subscriber("mavros/imu/water_pressure",
                     FluidPressure, PressureCallback)
...
if __name__ == '__main__':
    ...
    pub_depth = rospy.Publisher(
        'pid/depth/state', Float64, queue_size=10, tcp_nodelay=True)
